...
OGRErr OGREFALLayer::CreateNewTable()
{
    ...
    if (bNew && hTable == 0)
    {
        ...
        wchar_t * tablePath = pswzFileName;
...
OGRErr OGREFALLayer::ICreateFeature(OGRFeature *poFeature)
{
    ...
    if (err == OGRERR_NONE)
    {
        long nrecs = efallib->ExecuteInsert(hSession, hPreparedInsertStmt);
        ...
        if (nrecs == 1)
        {
            ...
            if (nLastFID <= 0)
            {
                ...
                std::wstring select = L"SELECT MAX(StringToNumber(MI_KEY,'999999999')) FROM \"";
                select += pwszFeatureClassName;
                select += L"\"";
                EFALHANDLE hMaxCursor = efallib->Select(hSession, select.c_str());
